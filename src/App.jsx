...
/**
 * Stable, working build — fixes:
 * - Removed accidental escape characters (e.g. className=\"...\") that caused
 *   "Expecting Unicode escape sequence" errors.
 * - Closed all JSX tags properly (e.g. </CardContent>). 
 * - Added a harmless `pageVariant = {}` so references are safe.
 * - Kept your previous layout, colors, gate modal style, pages and music logic
 *   (no autoplay; auto‑pause on Letter page; heart button to toggle music).
 * - FIX: Corrected `function GalleryPage()()` → `function GalleryPage()`.
 */
...
const motion = {
    ...
    span: (props: any) => <span {...omitMotion(props)} />,
    button: (props: any) => <button {...omitMotion(props)} />,
};
const AnimatePresence = ({ children }: any) => <>{children}</>;
const pageVariant: Record<string, unknown> = {};
...
const glass = "border border-rose-200/60 bg-white/80 backdrop-blur-xl shadow rounded-3xl";
...
function Card({ className, ...rest }: any) {
    return <div className={cx(glass, className)} {...rest} />;
}
const CardHeader = (p: any) => <div {...p} className={cx("px-6 pt-6", p.className)} />;
const CardContent = (p: any) => <div {...p} className={cx("px-6 pb-6", p.className)} />;
const CardTitle = ({ className, children, ...p }: any) => (
    <h2 {...p} className={cx("text-2xl font-semibold text-rose-700", className)}>
        {children}
    </h2>
);
const Input = (p: any) => (
...
function SparkleBg() {
    ...
    const floaters = useMemo(() => Array.from({ length: 18 }).map((_, i) => i), []);
    return (
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
            {dots.map((i) => (
                <span
                    ...
                    style={{ width: 3, height: 3, left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, opacity: 0.5 }}
                />
            ))}
            {floaters.map((i) => (
                <motion.span
...
function ConfettiLayer({ shots }: { shots: number }) {
    const [burst, setBurst] = useState < { id: string; x: number }[] > ([]);
    useEffect(() => {
        if (shots <= 0) return;
        const items = Array.from({ length: 14 }).map((_, idx) => ({ id: `${shots}-${idx}`, x: Math.random() * 100 }));
        setBurst(items);
    ...
    return (
        <div className="pointer-events-none absolute inset-0">
            {burst.map((b) => (
                <motion.div key={b.id} className="absolute text-xl" style={{ left: `${b.x}%`, top: "50%" }}>
                    {Math.random() > 0.5 ? "✨" : "💖"}
                </motion.div>
            ))}
        </div>
...
// -------------------- Gate Modal --------------------
function AnswerGate({ open, onSubmit, onClose }: { open: boolean; onSubmit: (ans: string) => void; onClose: () => void }) {
    ...
    return (
        <AnimatePresence>
            <motion.div className="fixed inset-0 z-50 bg-rose-300/40 backdrop-blur-sm grid place-items-center p-6" onClick={onClose}>
                <motion.div
                    className={cx(
                ...
                >
                    <div className="pointer-events-none absolute -top-2 left-6 text-2xl">✨</div>
                    <div className="pointer-events-none absolute -bottom-3 right-6 text-2xl">🌟</div>
                    <div className="px-6 pt-6 pb-4 text-center">
                        <div className="text-2xl">💖</div>
                        <h3 className="mt-2 text-lg sm:text-xl text-rose-800 font-semibold">Kaivalya tara mate kon che? (answer in one word)</h3>
                        <div className="mt-4 flex gap-2">
                            <Input autoFocus value={ans} onChange={(e: any) => setAns(e.target.value)} placeholder="e.g. jaan, prince, hero…" className="rounded-full border-rose-300 px-4 py-3" />
                            <Button onClick={submit} className="rounded-full px-5 py-3">Continue</Button>
                        </div>
...
// -------------------- Bottom Nav --------------------
function Nav({ page, setPage }: { page: PageKey; setPage: (p: PageKey) => void }) {
    const items = [
                ...
                <div className="flex justify-between gap-1 overflow-x-auto">
                    {items.map(({ id, icon, label }) => (
                        <button
                            key={id}
                            onClick={() => setPage(id)}
                            className={`flex flex-col items-center justify-center min-w-[84px] px-4 py-3 rounded-2xl transition-all ${page === id ? "bg-rose-500 text-white shadow-lg scale-[1.02]" : "text-rose-600 hover:bg-rose-50"
                                }`}
                            aria-label={label}
                        >
...
// -------------------- Pages --------------------
function HomePage({ setPage, openLetter }: { setPage: (p: PageKey) => void; openLetter: () => void }) {
    return (
        <motion.div className="mx-auto w-full max-w-4xl transform-gpu transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
            <Card className="rounded-[32px] border-2 border-rose-200/50 bg-white/80 backdrop-blur-2xl shadow-2xl">
                <CardHeader className="text-center">
                    ...
                    <p className="mt-3 text-rose-600">Today is your day — soft lights, calm skies, and all my love wrapped around you.</p>
                    <p className="text-rose-600">Explore your surprise pages below 💝</p>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Button className="h-14 rounded-[18px] w-full" onClick={openLetter}>💌 Open Letter</Button>
                        <Button className="h-14 rounded-[18px] w-full" onClick={() => setPage(PAGES.GALLERY)} variant="secondary">📷 My glowing Maharani</Button>
                        <Button className="h-14 rounded-[18px] w-full" onClick={() => setPage(PAGES.REASONS)} variant="outline">💖 19 Reasons</Button>
                        <Button className="h-14 rounded-[18px] w-full" onClick={() => setPage(PAGES.BUCKET)}>📝 Bucket List</Button>
                        <Button className="h-14 rounded-[18px] w-full" onClick={() => setPage(PAGES.SCRATCH)} variant="secondary">🎁 Scratch Cards</Button>
                        <Button className="h-14 rounded-[18px] w-full" onClick={() => setPage(PAGES.CERT)} variant="outline">📜 Certificate</Button>
                        <Button className="h-14 rounded-[18px] w-full" onClick={() => setPage(PAGES.WISHES)}>🎉 Make a wish</Button>
                    </div>
                    <div className="text-center text-sm text-rose-500">Tip: Use the floating menu at the bottom to switch pages anytime.</div>
                </CardContent>
            </Card>
        </motion.div>
    );
...
function LetterPage({ gfAnswer }: { gfAnswer: string | null }) {
            ...
            <Card>
                ...
                <CardContent className="space-y-4 text-rose-900/90 leading-relaxed">
                    <div className="text-center mb-2">
                        <h2 className="text-3xl sm:text-4xl font-[cursive] bg-gradient-to-r from-pink-500 to-rose-500 text-transparent bg-clip-text">To My Beautiful Priyu 💕</h2>
                        <p className="text-rose-500 italic text-lg mt-1">From Kaivu, with all my love</p>
                    </div>
                    <p>
                        <strong>Happiest 19th birthday to the most amazing person in my life!</strong> I wish many more happy, joyful years to come in your life — filled with love, laughter, and endless memories.
                    </p>
                    <p>
                        Today is your day — soft lights, calm skies, and all my love wrapped around you. You are truly special — not just to me, but to the whole world. Every wish you whisper deserves to come true, and I hope life gives you every single dream you chase.
                    </p>
                    <p>
                    ...
                    <div className="text-center mt-8">
                        <h3 className="text-2xl font-[cursive] bg-gradient-to-r from-pink-500 to-rose-500 text-transparent bg-clip-text">Forever yours, Kaivu 💕</h3>
                    </div>
                </CardContent>
            </Card>
        </motion.div>
            ...
            <Card className="p-6 mt-10 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
                ...
                <CardContent>
                    <div className="space-y-3">
                        {reasons.map((r, i) => (
                            <div key={i} className="p-4 rounded-2xl bg-rose-50 border border-white/60 text-rose-900/90 shadow-sm">
                                <div className="flex gap-3">
                                    <div className="text-2xl">{r.icon}</div>
                                    <div className="flex-1">
                                        <div className="text-xs text-rose-500">Reason #{i + 1}</div>
...
function BucketListPage() {
    type BL = { id: string; text: string; by: "priyu" | "kaivu"; completed: boolean };
    const [items, setItems] = useState < BL[] > (() => {
        try {
    ...
    const [addedBy, setAddedBy] = useState < "priyu" | "kaivu" > ("priyu");

    useEffect(() => {
        try { localStorage.setItem("bucket_list_v1", JSON.stringify(items)); } catch { }
    }, [items]);

    const add = () => {
        if (!newItem.trim()) return;
        const entry: BL = { id: Date.now().toString(), text: newItem.trim(), by: addedBy, completed: false };
        setItems((s) => [entry, ...s]);
        setNewItem("");
    };
    const toggle = (id: string) => setItems((it) => it.map((x) => (x.id === id ? { ...x, completed: !x.completed } : x)));
        ...
        <motion.div className="mx-auto max-w-3xl transform-gpu transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
            <Card className="p-6 mt-10 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
                ...
                <CardContent className="space-y-4">
                    <Textarea rows={3} placeholder="Add something you'd love to do together…" value={newItem} onChange={(e: any) => setNewItem(e.target.value)} />
                    <div className="flex gap-2">
                        <select value={addedBy} onChange={(e) => setAddedBy((e.target as HTMLSelectElement).value as "priyu" | "kaivu")} className="rounded-xl border-2 border-rose-200 bg-white/80 px-4 py-2.5 outline-none">
                            <option value="priyu">Added by Priyu 👑</option>
                            <option value="kaivu">Added by Kaivu 💕</option>
                        </select>
                        <Button className="flex-1" onClick={add}>Add to List</Button>
                    </div>
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-rose-600 mt-1">
                                                <span className="px-2 py-0.5 rounded-full bg-rose-200/60 text-rose-800">{item.by === "priyu" ? "👑 Priyu" : "💕 Kaivu"}</span>
                                            </p>
                                        </div>
...
function ScratchCard({ message, emoji }: { message: string; emoji: string }) {
    ...
    useEffect(() => {
        ...
        ctx.fillStyle = "#f9a8d4"; ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.fillStyle = "#fbcfe8"; for (let i = 0; i < 20; i++) ctx.fillRect(Math.random() * canvas.width, Math.random() * canvas.height, 4, 4);
        ctx.font = "bold 14px sans-serif"; ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.fillText("Scratch me! 💕", canvas.width / 4, canvas.height / 4);
    }, []);
    const scratch = (e: any) => {
        ...
        ctx.globalCompositeOperation = "destination-out"; ctx.beginPath(); ctx.arc(x * 2, y * 2, 40, 0, Math.PI * 2); ctx.fill();
        ...
        if (transparent / (pixels.length / 4) > 0.5 && !scratched) setScratched(true);
    };
    return (
        <div className="relative">
            ...
            {!scratched && (
                <canvas ref={canvasRef} className="absolute inset-0 w-full h-full cursor-pointer rounded-3xl" onMouseDown={() => setDrawing(true)} onMouseUp={() => setDrawing(false)} onMouseMove={scratch} onTouchStart={() => setDrawing(true)} onTouchEnd={() => setDrawing(false)} onTouchMove={scratch} />
            )}
        </div>
    );
}
...
function WishesPage() {
    ...
    type Wish = { id: string; text: string; by: "priyu" | "kaivu" };
    const [wish, setWish] = useState("");
    const [by, setBy] = useState < "priyu" | "kaivu" > ("priyu");
    ...
    useEffect(() => {
        try { localStorage.setItem("wishes_list_v1", JSON.stringify(items)); } catch { }
    }, [items]);

    const add = () => {
        const t = wish.trim();
        if (!t) return;
        const entry: Wish = { id: Date.now() + "", text: t, by };
        setItems((s) => [entry, ...s]);
        setWish("");
        fire();
    };
            ...
            <Card className={glass}>
                ...
                <CardContent className="space-y-4">
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <select
                            value={by}
                            onChange={(e) => setBy(e.target.value as "priyu" | "kaivu")}
...
function GalleryPage() {
    const photos = [
        { src: "https://i.imgur.com/Q35BSw6.png", caption: "Us, forever in one frame 💞" },
        ...
        { src: "https://i.imgur.com/u2HRKkm.jpeg", caption: "Exactly 19 years ago this cute angel was born ✨" },
    ...
    const next = () => setOpenIdx((i) => (i === null ? 0 : (i + 1) % photos.length));
    const prev = () => setOpenIdx((i) => (i === null ? 0 : (i - 1 + photos.length) % photos.length));
    const startX = useRef < number | null > (null);
    const onTouchStart = (e: React.TouchEvent) => (startX.current = e.touches[0].clientX);
    const onTouchEnd = (e: React.TouchEvent) => {
        const x = startX.current; if (x === null) return; startX.current = null;
        const diff = e.changedTouches[0].clientX - x; if (diff > 40) prev(); if (diff < -40) next();
    };
    return (
        <motion.div className="mx-auto max-w-5xl">
            <Card className="p-6 mt-10 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
                ...
                <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {photos.map((p, i) => (
                            <button key={i} className="group relative" onClick={() => setOpenIdx(i)}>
                                <img src={p.src} alt={p.caption} className="w-full h-40 md:h-44 object-cover rounded-2xl border border-rose-200/60 shadow" />
            ...
            {openIdx !== null && (
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm grid place-items-center p-4 transition-opacity duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]" onClick={close}>
                    <div className="relative w-full max-w-3xl transform-gpu transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]" onClick={(e) => e.stopPropagation()}>
                        <img src={photos[openIdx].src} alt={photos[openIdx].caption} className="w-full max-h-[70vh] object-contain rounded-2xl" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} />
                        <div className="mt-3 text-center text-white text-sm md:text-base">{photos[openIdx].caption}</div>
                        <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-between px-2">
                            <button onClick={prev} className="text-white/90 bg-black/30 hover:bg-black/50 w-10 h-10 rounded-full grid place-items-center">‹</button>
                            <button onClick={next} className="text-white/90 bg-black/30 hover:bg-black/50 w-10 h-10 rounded-full grid place-items-center">›</button>
                        </div>
                        <button onClick={close} className="absolute -top-3 -right-3 bg-rose-500 text-white w-8 h-8 rounded-full">✕</button>
                    </div>
                </div>
            )}
        </motion.div>
            ...
            <div className="max-w-3xl mx-auto text-center">
                ...
                <div className="mx-auto max-w-3xl rounded-3xl border-[6px] border-rose-300/80 bg-white/70 shadow-lg p-4 relative">
                    ...
                    <div className="rounded-2xl border-2 border-rose-200 bg-gradient-to-b from-rose-50/70 to-pink-50/60 px-8 sm:px-12 py-10 text-rose-700">
                        <h2 className="text-3xl sm:text-4xl font-serif font-extrabold text-rose-700 mb-8">Certificate of Love</h2>
                        <p className="text-rose-500">This certifies that</p>
                        <p className="text-3xl sm:text-4xl font-extrabold text-rose-700 my-4">Priyu <span className="align-middle">👑</span></p>
                        <p className="max-w-xl mx-auto leading-relaxed">is the most beautiful, caring, and amazing person in the entire world and is deeply loved by Kaivu</p>
                        <p className="text-2xl font-semibold text-rose-600 mt-8">Forever and Always ✨</p>
                        <hr className="my-8 border-rose-200" />
                        <p className="text-sm text-rose-400 mb-4">Dated: {new Date().toLocaleDateString()}</p>
...
function PageFade({ dep, children }: { dep: any; children: React.ReactNode }) {
...
export default function App() {
    ...
    const [gfAnswer, setGfAnswer] = useState < string | null > (null);
    ...
    useEffect(() => { const el = audioRef.current; if (el) el.volume = 0.7; }, []);
    useEffect(() => { const el = audioRef.current; if (!el) return; if (page === PAGES.LETTER) { el.pause(); setIsPlaying(false); } }, [page]);
    const toggleAudio = async () => { const el = audioRef.current; if (!el) return; if (isPlaying) { el.pause(); setIsPlaying(false); } else { try { await el.play(); setIsPlaying(true); } catch { } } };

    const openLetter = () => setShowGate(true);
    const handleSubmitAnswer = (ans: string) => { setGfAnswer(ans); setShowGate(false); setPage(PAGES.LETTER); };

    const renderPage = () => {
        switch (page) {
            case PAGES.HOME: return <HomePage setPage={setPage} openLetter={openLetter} />;
            case PAGES.LETTER: return <LetterPage gfAnswer={gfAnswer} />;
            case PAGES.GALLERY: return <GalleryPage />;
            case PAGES.REASONS: return <ReasonsPage />;
            case PAGES.BUCKET: return <BucketListPage />;
            case PAGES.SCRATCH: return <ScratchCardsPage />;
            case PAGES.CERT: return <CertificatePage />;
            case PAGES.WISHES: return <WishesPage />;
            default: return <HomePage setPage={setPage} openLetter={openLetter} />;
        }
    };
    ...
    return (
        <div className={cx("min-h-screen relative", appGradient)}>
            ...
            <motion.button onClick={toggleAudio} aria-label={isPlaying ? "Pause music" : "Play music"} className="fixed bottom-24 right-4 z-40 grid place-items-center w-11 h-11 rounded-full shadow-lg text-white transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] active:scale-95" style={{ background: "linear-gradient(135deg, #fb7185, #f43f5e)" }}>
